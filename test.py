...
def get_opt():
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='./checkpointss/')
...
def test(opt, seg):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            print(inputs['parse_agnostic'].size())
            ...
            # print(type(parse_agnostic_down.cpu()))
            # torch.save(parse_agnostic_down.cpu(), 'parse_agnostic_down.pt')
            pose_down = F.interpolate(pose, size=(256, 192), mode='bilinear')
            # print(type(pose_down.cpu()))
            # torch.save(pose_down.cpu(), 'pose_down.pt')
            c_masked_down = F.interpolate(c * cm, size=(256, 192), mode='bilinear')
            # print(type(c_masked_down.cpu()))
            # torch.save(c_masked_down.cpu(), 'c_masked_down.pt')
            cm_down = F.interpolate(cm, size=(256, 192), mode='bilinear')
            # print(cm_down.size(), pose_down.size(), c_masked_down.size(), cm_down.size())
            # torch.save(cm_down.cpu(), 'cm_down.pt')
            c_down = F.interpolate(c, size=(256, 192), mode='bilinear')
            # torch.save(c_down.cpu(), 'c_down.pt')
            seg_input = torch.cat((cm_down, c_masked_down, parse_agnostic_down, pose_down, gen_noise(cm_down.size()).cuda()), dim=1)
            # print(type(seg_input.cpu()))
            torch.save(seg_input.cpu(), 'I' + img_names[0].split('.')[0] + '_C' + c_names[0].split('.')[0] + '_seg_input.pt')
            parse_pred_down = seg(seg_input)
            # print(type(parse_pred_down.cpu()))
            torch.save(parse_pred_down.cpu(), 'I' + img_names[0].split('.')[0] + '_C' + c_names[0].split('.')[0] + '_parse_pred_down.pt')
